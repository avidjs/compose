...
module.exports = function compose(middleware) {
  ...
  return function stack(ctx, next) {
     ...
     * incremented by one for comparison in order to ensure that the middleware
     * is traversed one at a time.
     * @type {Number}
     */
    let counter = -1;
    ...
    /**
     * Recurseively executes the next middleware in the stack with the given
